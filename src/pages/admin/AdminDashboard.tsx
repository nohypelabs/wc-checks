    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-600 via-blue-500 to-cyan-500 lg:bg-gradient-to-r lg:from-slate-50 lg:to-slate-100 pb-20 lg:pb-0">
      ...
      <div className="max-w-7xl mx-auto px-3 lg:px-6 pt-3 lg:pt-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-3 mb-3 lg:mb-4">
          <Card className="p-2.5 lg:p-3 bg-white shadow-xl border-0 lg:shadow-sm">
            <div className="flex items-center gap-1.5 mb-1">
              ...
              {stats && stats.inspectionGrowth !== 0 && (
                <div className={`flex items-center gap-0.5 text-[10px] lg:text-xs font-medium ${
                  stats.inspectionGrowth > 0 ? 'text-green-600' : 'text-red-600'
                }`}>
                  {stats.inspectionGrowth > 0 ? <TrendingUp className="w-2.5 h-2.5" /> : <TrendingDown className="w-2.5 h-2.5" />}
                  <span>{Math.abs(stats.inspectionGrowth)}%</span>
                </div>
              )}
            </div>
            <div className="text-xl lg:text-lg font-bold text-gray-900 leading-none">{stats?.todayInspections || 0}</div>
            <div className="text-[10px] lg:text-xs text-gray-500 mt-0.5">Hari Ini</div>
          </Card>

          <Card className="p-2.5 lg:p-3 bg-white shadow-xl border-0 lg:shadow-sm">
            <div className="flex items-center gap-1.5 mb-1">
              <div className="w-6 h-6 lg:w-8 lg:h-8 bg-green-100 rounded-md lg:rounded-lg flex items-center justify-center">
                <Calendar className="w-3 h-3 lg:w-4 lg:h-4 text-green-600" />
              </div>
            </div>
            <div className="text-xl lg:text-lg font-bold text-gray-900 leading-none">{stats?.inspections7d || 0}</div>
            <div className="text-[10px] lg:text-xs text-gray-500 mt-0.5">7 Hari</div>
          </Card>
          ...
          <Card className="p-2.5 lg:p-3 bg-white shadow-xl border-0 lg:shadow-sm">
            ...
            <div className="text-xl lg:text-lg font-bold text-gray-900 leading-none">{stats?.inspections30d || 0}</div>
            <div className="text-[10px] lg:text-xs text-gray-500 mt-0.5">30 Hari</div>
          </Card>

          <Card className="p-2.5 lg:p-3 bg-white shadow-xl border-0 lg:shadow-sm">
            <div className="flex items-center gap-1.5 mb-1">
              <div className="w-6 h-6 lg:w-8 lg:h-8 bg-orange-100 rounded-md lg:rounded-lg flex items-center justify-center">
                <BarChart3 className="w-3 h-3 lg:w-4 lg:h-4 text-orange-600" />
              </div>
            </div>
            <div className="text-xl lg:text-lg font-bold text-gray-900 leading-none">{stats?.totalInspections || 0}</div>
            <div className="text-[10px] lg:text-xs text-gray-500 mt-0.5">Total</div>
          </Card>
        </div>
        ...
        <div className="lg:grid lg:grid-cols-3 lg:gap-4 lg:items-end">
          ...
          {isAdmin && (
            <div className="mb-3 lg:mb-0 lg:col-span-2">
              <h2 className="text-sm lg:text-lg font-bold text-white lg:text-gray-800 lg:mb-2 mb-2">Management</h2>
          ...
          <div className={isAdmin ? 'lg:col-span-1' : 'lg:col-span-3 lg:max-w-md lg:mx-auto'}>
            <Card className="bg-white shadow-xl border-0 lg:shadow-sm lg:h-full" padding="none">
              <div className="px-3 py-2 lg:px-3 lg:py-2.5 border-b border-gray-100">
                <div className="flex items-center gap-1.5">
                  <Activity className="w-3.5 h-3.5 text-blue-600" />
                  <h3 className="font-semibold text-gray-900 text-xs lg:text-sm">System Overview</h3>
                </div>
              </div>
              <div className="divide-y divide-gray-50 lg:divide-y-0 lg:divide-x lg:divide-gray-100 lg:flex lg:flex-col">
                <div className="flex items-center justify-between px-3 py-2 lg:px-3 lg:py-2.5">
                  <span className="text-[11px] lg:text-xs text-gray-500">Total Inspections</span>
                  <span className="text-xs lg:text-sm font-bold text-gray-900">{stats?.totalInspections || 0}</span>
                </div>
                <div className="flex items-center justify-between px-3 py-2 lg:px-3 lg:py-2.5">
                  <span className="text-[11px] lg:text-xs text-gray-500">Active Locations</span>
                  <span className="text-xs lg:text-sm font-bold text-gray-900">{stats?.totalLocations || 0}</span>
                </div>
                <div className="flex items-center justify-between px-3 py-2 lg:px-3 lg:py-2.5">
                  <span className="text-[11px] lg:text-xs text-gray-500">Today's Activity</span>
                  <span className="text-xs lg:text-sm font-bold text-gray-900">{stats?.todayInspections || 0}</span>
                </div>
              </div>
            </Card>
          </div>
