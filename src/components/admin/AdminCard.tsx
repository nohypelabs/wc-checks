...
export const AdminCard = ({
  ...
  const desktopIcon: Record<string, string> = {
    blue: 'lg:bg-blue-100',
    purple: 'lg:bg-purple-100',
    green: 'lg:bg-green-100',
    orange: 'lg:bg-orange-100',
    red: 'lg:bg-red-100',
    cyan: 'lg:bg-cyan-100',
  };

  const desktopIconText: Record<string, string> = {
    blue: 'lg:text-blue-600',
    purple: 'lg:text-purple-600',
    green: 'lg:text-green-600',
    orange: 'lg:text-orange-600',
    red: 'lg:text-red-600',
    cyan: 'lg:text-cyan-600',
  };
  ...
  return (
    <button
      ...
      className="
        ...
        lg:bg-white lg:border-gray-100 lg:shadow-sm lg:backdrop-blur-none
    ...
    >
      ...
      <div className="mb-1 lg:mb-1.5">
        <h3 className="text-xs lg:text-sm font-bold text-white lg:text-gray-900 leading-tight">{title}</h3>
        <p className="text-[10px] lg:text-xs text-white/50 lg:text-gray-500 mt-0.5 hidden lg:block">{description}</p>
      </div>

      {/* Footer */}
      <div className="flex items-center justify-between">
        {count !== undefined && (
          <div className="text-base lg:text-lg font-bold text-white lg:text-gray-900">{count}</div>
        )}
        <div className={`
        ...
        `}>
          <span>Manage</span>
          <ChevronRight className="w-3 h-3 lg:w-3.5 lg:h-3.5 text-white/50 lg:text-gray-400" />
        </div>
      </div>
    </button>
  );
