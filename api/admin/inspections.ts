  ...
  try {
    ...
    if (inspectionId) {
      const { data: inspection, error } = await supabase
        ...
        .select('id, inspection_date, inspection_time, overall_status, responses, photo_urls, location_id, user_id, notes, submitted_at, duration_seconds, locations!inner(id, name, floor, buildings(name)), users!inspection_records_user_id_fkey(full_name, email)')
        .eq('id', inspectionId)
        .single();
